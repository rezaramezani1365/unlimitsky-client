...
function usk_service_list_row(array $order)
{
    ...
    } elseif ($volumeGb > 0 && in_array((string) ($order['protocol'] ?? ''), array('wireguard', 'openvpn', 'xray', 'l2tp', 'cisco'), true)) {
        $usageDisplay = __('services_usage_pending');
        $usageNeedsSync = true;
    }
    ...
    return array(
        ...
        'volume' => $volumeGb,
        'volume_label' => usk_format_plan_gb($volumeGb),
        'date' => (int) ($order['date'] ?? 0),
        'date_label' => usk_format_plan_days((int) ($order['date'] ?? 0)),
        'protocol' => (string) ($order['protocol'] ?? ''),
        'status' => $status,
